...
class Roadmap:    
    ...
    def computeConnectedComponents(self, u, nbr): # implement this to compute the connected components of the undirected graph
        if len(u.edges) == 0: return False
        for e in u.edges:
            for e1 in self.vertices[e.id].edges:
                if nbr.id == e1.id: return True
                for e2 in self.vertices[e1.id].edges:
                    if nbr.id == e2.id: return True
                    for e3 in self.vertices[e2.id].edges:
                        if nbr.id == e3.id: return True
                        for e4 in self.vertices[e3.id].edges:
                            if nbr.id == e4.id: return True
                            for e5 in self.vertices[e4.id].edges:
                                if nbr.id == e5.id: return True
