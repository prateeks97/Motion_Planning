...
class ReedsSheppPath(object):
    ...
    @classmethod
    def gen_path(cls, s, path, r, section=True):
...
def test3():
    plt.figure()
    start = [1, 1, 0 / 180.0 * pi]
    end = [3, 4, 0 / 180.0 * pi]
    r = 0.25
    rspath = ReedsSheppPath(start, end, r)
    rspath.calc_paths()
    path, cost = rspath.get_shortest_path()
    print(path, cost, "yay")
    title = ''
    for p in path:
        title += p[0] + ('+' if p[1] > 0 else '-')
    ...
    draw_point(start)
    draw_point(end)
    xs, ys, phi = ReedsSheppPath.gen_path(start, path, r)
    # print(len(xs), len(ys), len(phi), "pasdf")
    for i in range(len(xs)):
        # print(xs[i], ys[i], phi[i], "kamariya lollipop lagelu")
        plt.plot(xs[i], ys[i])
    plt.axis("equal")
    plt.show()
