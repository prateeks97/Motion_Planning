...
#
from graphvisual import *

class RoadmapVertex:
    """A class for storing the vertices of your roadmap """

    def __init__(self, q, id, g, p, path, path_type):
...
class Roadmap:
    ...
    def getVisualVertices(self):
        return self.visualvertices

    def getNrVertices(self):
        return len(self.vertices)

    def getNrVisualVertices(self):
        return len(self.visualvertices)

    def addVertex(self, q, g, p, path, path_type):  # adds a configuration to the roadmap
        v = RoadmapVertex(q, len(self.vertices), g, p, path, path_type)
        self.vertices.append(v)
        return self.vertices[-1]

    def addVisualVertex(self, q, g, p):  # adds a configuration to the roadmap
        v = VisualVertex(q, len(self.visualvertices), g, p)
        self.visualvertices.append(v)
        return self.visualvertices[-1]
    ...
    def addVisualEdge(self, u, v, dist, path = []): # adds an edge between two vertices, if such vertices exist
        u_id = u.id
        v_id = v.id
        if u_id!=v_id and 0<=u_id<len(self.visualvertices) and 0<=v_id<len(self.visualvertices):
            if (self.directed):
                return self.visualvertices[u_id].addEdge(v_id, dist, path)
            elif self.visualvertices[u_id].addEdge(v_id, dist, path):
                return self.visualvertices[v_id].addEdge(u_id, dist, list(reversed(path)))
        return False
